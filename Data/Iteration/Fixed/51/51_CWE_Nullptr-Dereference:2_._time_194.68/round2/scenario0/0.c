...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 28 by call to `getnode()`, is not freed after the last access at line 42, column 13.
 Example: 
  40.         lptr->next = getnode();
  41. 
  42.         if (lptr->next == NULL) {
                  ^
