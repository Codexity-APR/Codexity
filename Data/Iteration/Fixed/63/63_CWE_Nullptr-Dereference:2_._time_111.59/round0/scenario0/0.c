...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 71.
 Example: 
  70.     // Create a new node
  71.     node* newNode = malloc(sizeof(node));
  72.     strcpy(newNode->entry, entry);
          ^
...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 37.
 Example: 
  38.     
  39.     // Initialize the head node
  40.     strcpy(head->entry, "");
          ^
  41.     head->next = NULL;
  42.
