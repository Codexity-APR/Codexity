...
Error: Null Dereference   pointer `newNode` last assigned on line 102 could be null and is dereferenced at line 103, column 5.
 Example: 
  101. {
  102.     node* newNode = (node*)malloc(sizeof(node));
  103.     newNode->data = d;
           ^
  104.     newNode->prev = NULL;
  105.     newNode->next = head;

Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 102.
 Example: 
  101. {
  102.     node* newNode = (node*)malloc(sizeof(node));
  103.     newNode->data = d;
