...
Error: Null Dereference   pointer `combo` last assigned on line 23 could be null and is dereferenced at line 45, column 26.
 Example: 
  43. 
  44.         // print the result
  45.         printf("%lld\n", combo[passlength].skips);
                               ^
  46.     }
  47.
...
long long magical(password *combo, int passlength) {
    combo[passlength].skips = 0;
    FindVariations(passlength, passlength, combo, 0, 0);
    return combo[passlength].skips;
}