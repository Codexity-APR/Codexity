...
Error: Resource Leak   resource acquired by call to `fopen()` at line 22, column 16 is not released after line 46, column 46.
 Example: 
  44.         free(data);
  45.     } else {
  46.         printf("Invalid magic number: %u\n", magic_number);
                                                   ^
  47.         return 1;
  48.     }

 Error type: CWE-775 at line: 47. Error message: Resource leak: fp
...
void load_mnist_images(char *filename, uint8_t **images, uint32_t *num_images) {
    // Implement the function to load the MNIST images
    // ...
}
void load_mnist_labels(char *filename, uint8_t **labels, uint32_t *num_labels) {
    // Implement the function to load the MNIST labels
    // ...
}
void print_images(uint8_t *images, uint32_t num_images) {
    // Implement the function to print the MNIST images
    // ...
}
void print_labels(uint8_t *labels, uint32_t num_labels) {
    // Implement the function to print the MNIST labels
    // ...
}