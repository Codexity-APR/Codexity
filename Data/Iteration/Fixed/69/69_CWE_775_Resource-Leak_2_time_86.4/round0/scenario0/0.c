...
Error: Resource Leak   resource acquired by call to `fopen()` at line 22, column 16 is not released after line 46, column 45.
 Example: 
  44.         free(data);
  45.     } else {
...
void load_mnist_images(char *filename, uint8_t **images, uint32_t *num_images) {
    // Implementation of loading MNIST images from file
    // ...
}
void load_mnist_labels(char *filename, uint8_t **labels, uint32_t *num_labels) {
    // Implementation of loading MNIST labels from file
    // ...
}
void print_images(uint8_t *images, uint32_t num_images) {
