...
Error: Null Dereference   pointer `params` last assigned on line 66 could be null and is dereferenced at line 73, column 9.
 Example: 
  ...
  73.         params[i].idx = i;
              ^
  74.         if (0 != pthread_create(&ids[i], NULL, partial_sum, (void *)&params[i]))
  75.         {

 Error type: CWE-686 at line: 76. Error message: %lld in format string (no. 1) requires 'long long' but the argument type is 'unsigned long long'.

 Error type: CWE-686 at line: 87. Error message: %lld in format string (no. 1) requires 'long long' but the argument type is 'unsigned long long'.
