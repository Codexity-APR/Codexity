...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 16.
 Example: 
  15. clientdata* createClient(char name[254], int birthday, int landline, int cellphone, int clientnum) {
  16.     clientdata* client = (clientdata*)malloc(sizeof(clientdata));
  17.     strcpy(client->name, name);
          ^
  18.     client->birthday = birthday;
  19.     client->landline = landline;
