...
void* second_thread(void* arg) {
    while (true) {
        ...
        memcpy(array, fifo.buffer[fifo.tail], ARRAY_LENGTH * sizeof(uint8_t));
...
int main() {
    ...
    if (pthread_create(&thread1, NULL, main_thread, NULL) != 0) {
        printf("Failed to create main thread\n");
        return 1;
    }

    // create the second thread
    if (pthread_create(&thread2, NULL, second_thread, NULL) != 0) {
