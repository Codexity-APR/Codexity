...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 17 indirectly during the call to `addSpecific()`in call to `addSpecific()` .
 Example: 
  ...
  39.     addSpecific(head, 8);
          ^
  40.     
  41.     printf("LinkedList: ");
...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 34 by call to `malloc`, is not freed after the last access at line 42, column 5.
 Example: 
  40.     
  41.     printf("LinkedList: ");
  42.     printList(head);
