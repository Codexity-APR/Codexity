...
Error: Resource Leak   resource acquired by call to `open()` at line 15, column 14 is not released after line 39, column 9.
 Example: 
  37.     // Unmap the file from memory
  38.     if (munmap(fileData, sb.st_size) == -1) {
  39.         perror("munmap");
              ^
  40.         return 1;
  41.     }
...
Error: Resource Leak   resource acquired by call to `open()` at line 15, column 14 is not released after line 31, column 9.
 Example: 
  29.     char *fileData = (char *) mmap(0, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
  30.     if (fileData == MAP_FAILED) {
  31.         perror("mmap");
              ^
  32.         return 1;
  33.     }
...
int main(int argc, char *argv[]) {
    ...
    char *fileData = (char *) mmap(0, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
