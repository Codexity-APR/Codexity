...
Error: Resource Leak   resource acquired by call to `open()` at line 15, column 14 is not released after line 42, column 9.
 Example: 
  40.     // Cleanup: unmap the file from memory and close the file descriptor
  41.     if (munmap(file_data, sb.st_size) == -1) {
  42.         perror("munmap");
              ^
  43.         return 1;
  44.     }
...
Error: Resource Leak   resource acquired by call to `open()` at line 15, column 14 is not released after line 31, column 9.
 Example: 
  29.     char *file_data = (char *) mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
int main(int argc, char *argv[]) {
    ...
    // Cleanup: unmap the file from memory and close the file descriptor
