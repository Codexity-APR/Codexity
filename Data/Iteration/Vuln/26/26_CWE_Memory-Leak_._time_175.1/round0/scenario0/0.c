...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 18 by call to `strAlloc()`, is not freed after the last access at line 26, column 9.
 Example: 
  24.     while ((ch = getchar()) != '\n') {
  25.         (*str)[len++] = ch;
  26.         *str = realloc(*str, len + 1);
              ^
  27. 
  28.         if (!*str) {
