...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 64 by call to `parse_string()`, is not freed after the last access at line 73, column 5.
 Example: 
  71.         free(tokens[i]);
  72.     }
  73.     free(tokens); // Free the tokens array
          ^
  74.     return 0;
  75. }
