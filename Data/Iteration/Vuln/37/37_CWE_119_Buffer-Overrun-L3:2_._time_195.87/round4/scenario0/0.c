...
Error: Buffer Overrun L3   Offset: [-1, 2499] Size: 2500.
 Example: 
  51.         }
  52.         // Update max count and most frequent word
  53.         if (count[wordCount-1] > maxCount) {
                  ^
  54.             maxCount = count[wordCount-1];
  55.             strncpy(mostFreqWord, words[wordCount-1], sizeof(mostFreqWord) - 1);
