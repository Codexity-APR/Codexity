...
Error: Buffer Overrun L3   Offset: [-1, 2499] Size: 2500.
 Example: 
  50.         }
  51.         // Update max count and most frequent word
  52.         if (count[wordCount-1] > maxCount) {
                  ^
  53.             maxCount = count[wordCount-1];
  54.             strncpy(mostFreqWord, words[wordCount-1], MAX_WORD_LENGTH - 1);
...
int main(int argc, char *argv[])
{
    ...
    while (fscanf(fp, "%49s", word) == 1) {
        ...
        if (count[wordCount-1] > maxCount) {
            ...
            strncpy(mostFreqWord, words[wordCount-1], MAX_WORD_LENGTH - 1);
            mostFreqWord[MAX_WORD_LENGTH - 1] = '\0';
        }
    }
    // Print the results
