...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 54 by call to `createClient()`, is not freed after the last access at line 60, column 5.
 Example: 
  58.     addClient(&head, client2);
  59.     
  60.     printClients(head);
          ^
  61.     
  62.     return 0;
...
Error: Buffer Overrun L1   Offset added: 254 Size: 40 by call to `createClient`.
 Example: 
  52.     clientdata* head = NULL;
  53.     
  54.     clientdata* client1 = createClient("John", 1990, 1234567, 987654321);
                                ^
  55.     clientdata* client2 = createClient("Anna", 1985, 7654321, 123456789);
  56.
...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 16.
 Example: 
  ...
  17.     strncpy(newClient->name, name, 254);
          ^
