...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 35 by call to `malloc`, is not freed after the last access at line 47, column 9.
 Example: 
  45. 
  46.         // Append NULL at the end of input_args to indicate end of arguments
  47.         input_args = realloc(input_args, (num_args + 1) * sizeof(char *));
              ^
  48.         if (input_args == NULL) {
  49.             perror("Memory allocation failed");

Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 35 by call to `malloc`, is not freed after the last access at line 29, column 13.
 Example: 
  27. 
  28.         while (token != NULL) {
  29.             input_args = realloc(input_args, (num_args + 1) * sizeof(char *));
                  ^
  30.             if (input_args == NULL) {
