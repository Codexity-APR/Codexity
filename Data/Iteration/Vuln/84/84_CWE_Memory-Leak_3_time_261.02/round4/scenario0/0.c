...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 81 by call to `create_node()`, is not freed after the last access at line 92, column 3.
 Example: 
  90.   add_child(child1, child4);
  91.   add_child(child1, child5);
  92.   print_tree(root, 0);
        ^
  93.   free_tree(root); // Free the memory allocated for the tree
  94.   return 0;
...
void free_tree(struct node *root) {
  ...
  struct node *current = root->first_child;
  while (current != NULL) {
    struct node *next_sibling = current->next_sibling;
    free_tree(current);
...
int main() {
  ...
  print_tree(root, 0);
  free_tree(root); // Free the memory allocated for the tree
  return 0;
}