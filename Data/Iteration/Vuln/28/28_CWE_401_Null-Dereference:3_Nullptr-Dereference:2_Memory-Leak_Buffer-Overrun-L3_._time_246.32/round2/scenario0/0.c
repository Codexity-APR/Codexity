...
Error: Memory Leak   Pulse found a potential memory leak. Memory dynamically allocated at line 45 by call to `malloc`, is not freed after the last access at line 74, column 13.
 Example: 
  72.         // Resize the tokens array
  73.         char** new_tokens = realloc(tokens, (i + 2) * sizeof(char*));
  74.         if (new_tokens == NULL) {
                  ^
  75.             free(new_token);
