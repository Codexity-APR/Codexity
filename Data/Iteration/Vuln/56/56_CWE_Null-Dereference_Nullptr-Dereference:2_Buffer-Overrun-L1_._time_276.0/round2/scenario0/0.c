...
Error: Null Dereference   pointer `combo` last assigned on line 24 could be null and is dereferenced by call to `FindVariations()` at line 39, column 9.
 Example: 
  37. 
  38.         // find variations and output the result
  39.         FindVariations(passlength, strength, combo, 0, 0);
              ^
  40.         printf("%lld\n", magical(combo, passlength));
  41.     }

Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 24in call to `FindVariations()` .
 Example: 
  37. 
  38.         // find variations and output the result
  39.         FindVariations(passlength, strength, combo, 0, 0);
...
Error: Nullptr Dereference   Pulse found a potential null pointer dereference  on line 24.
 Example: 
  30.         // input the letters for password
  31.         for (j = 0; j < passlength; j++) {
  32.             scanf(" %c", &combo[j].letters[0]);
                  ^
  33.         }
  34.
