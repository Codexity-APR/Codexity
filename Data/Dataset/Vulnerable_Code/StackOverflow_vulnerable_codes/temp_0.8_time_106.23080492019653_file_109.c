...
int main(int argc, char **argv) {
  ...
  while (sent < 100) {
    ...
    // Send packet
    if (sendto(sockfd, packet, packet_size, 0, (struct sockaddr*)&servaddr, ls) == -1) {
      perror("sendto failed");
    }

    sent++;
    sleep(1);  // Add delay between sending packets

    // Receive packet
    if ((rf = recvfrom(sockfd, buffer, packet_size, 0, (struct sockaddr*)&servaddr, &ls)) == -1) {
