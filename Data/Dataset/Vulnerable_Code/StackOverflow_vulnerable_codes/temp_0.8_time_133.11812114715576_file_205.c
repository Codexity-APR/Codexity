...
node* removeNode(node* head, int d)
{
    ...
    while (current != NULL)
    {
        if (current->data == d)
        {
            if (current->prev == NULL) // Removing the head node
            {
                head = current->next;
                head->prev = NULL;
            }
            else if (current->next == NULL) // Removing the tail node
            {
...
int locInList(node* head, int x)
{
    ...
    return -1; // Assuming -1 represents the element not found in the list
}