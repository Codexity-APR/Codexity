...
int main(int argc, char **argv) {
    ...
    uint32_t num_bytes_read = fread(&magic_number, sizeof(magic_number), 1, fp);
    if (num_bytes_read != 1) {
    ...
    num_bytes_read = fread(&num_items, sizeof(num_items), 1, fp);
    if (num_bytes_read != 1) {
    ...
    if (magic_number == 2051) {
        ...
        fseek(fp, 8, SEEK_SET);
        num_bytes_read = fread(&num_rows, sizeof(num_rows), 1, fp);
        if (num_bytes_read != 1) {
        ...
        num_bytes_read = fread(&num_cols, sizeof(num_cols), 1, fp);
        if (num_bytes_read != 1) {
