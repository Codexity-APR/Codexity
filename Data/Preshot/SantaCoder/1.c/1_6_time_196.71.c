...
int main(int argc, char **argv) {
  ...
  while (sent < 100) {
    ...
    // Send the packet
    if (sendto(sockfd, buffer, packet_size, 0, (struct sockaddr*) &servaddr, sizeof(servaddr)) != packet_size) {
      perror("sendto");
      return 1;
    }

    // Receive the packet
    if (recvfrom(sockfd, buffer, packet_size, 0, (struct sockaddr*) &servaddr, &ls) != packet_size) {
      perror("recvfrom");
      return 1;
    }
