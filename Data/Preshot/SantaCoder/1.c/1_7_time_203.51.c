...
int main(int argc, char **argv) {
  ...
  while (sent < 100) {
    ...
    // Send the packet
    if (sendto(sockfd, packet, packet_size, 0, (struct sockaddr *) &servaddr, sizeof(servaddr)) != packet_size) {
      perror("sendto");
      return 1;
    }

    // Receive the packet
    if (recvfrom(sockfd, packet, packet_size, 0, (struct sockaddr *) &servaddr, &ls) != packet_size) {
      perror("recvfrom");
      return 1;
    }

    // Checksum
    if (icmphdr->checksum != htons(in_cksum((unsigned short*) icmphdr, sizeof(struct icmp_header) + payload_size))) {
      perror("checksum");
      return 1;
    }
