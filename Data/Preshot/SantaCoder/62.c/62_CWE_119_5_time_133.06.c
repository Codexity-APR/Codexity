...
int main() {
    ...
    scanf("%s", word);
    length = strlen(word);
    printf("The word you entered is %d letters long.\n", length);
    printf("Let's start guessing!\n");
    ...
    while (!is_game_over(guesses_left, word_guessed)) {
        ...
        int found = 0;
        for (int i = 0; i < length; i++) {
            if (word[i] == guess && !letters_guessed[i]) {
                letters_guessed[i] = 1;
                found = 1;
            }
        }
    ...
    if (word_guessed) {
        printf("Congratulations! You guessed the word \"%s\" with %d guesses remaining.\n", word, guesses_left);
    } else {
