...
void* main_thread(void* arg) {
    while (true) {
        ...
        // increase the count of the elements in the buffer
        fifo.count++;
...
void* consumer_thread(void* arg) {
    while (true) {
        // get an array from the circular buffer
        pthread_mutex_lock(&fifo.lock);
        while (fifo.count == 0) {
        ...
        // decrease the count of the elements in the buffer
        fifo.count--;
...
int main(int argc, char* argv[]) {
    ...
    pthread_create(&main_thread, NULL, main_thread, NULL);
    // create the consumer thread
    pthread_t consumer_thread;
    pthread_create(&consumer_thread, NULL, consumer_thread, NULL);
