...
int main() {
    ...
    for (i = 0; i < cases; i++) {
        ...
        if(scanf("%d", &passlength) != 1) {
            return -1;
        }

        // allocate memory for password struct
        ...
        for (j = 0; j < passlength; j++) {
            if (scanf(" %c", &combo[j].letters[j]) != 1) {
                free(combo);
                return -1;
            }
        }
        ...
        // free memory for password struct
        free(combo);
        combo = NULL;
    }
...
long long magical(password *combo, int passlength) {
    ...
    for (i = 0; i < passlength; i++) {
        // if the letter is in the password, add to combinations
