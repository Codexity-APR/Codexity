...
int main() {
  ...
  char guessed_letters[MAX_WORD_LENGTH] = {0}; // set all elements to 0
  int num_guesses = 0;
...
#ifdef DEBUG
  printf("Word list index: %d\n", word_index);
#endif

  // Copy the word into the word array
  strncpy(word, word_list[word_index], MAX_WORD_LENGTH); // ensure the size of the word is not more than MAX_WORD_LENGTH
