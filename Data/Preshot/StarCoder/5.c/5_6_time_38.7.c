...
int main(void)
{
    ...
    if (scanf("%49s", reference) != 1) // use scanf with field width limit to prevent buffer overflow
    {
    ...
    if (scanf("%49s", query) != 1) // use scanf with field width limit to prevent buffer overflow
    {
        printf("Error reading input\n");
        return 1;
    }
