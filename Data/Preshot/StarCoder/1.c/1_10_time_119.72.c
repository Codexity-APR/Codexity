...
int main(int argc, char **argv) {
  ...
  while (sent < 100) {
    ...
    // Allocate memory for the buffer and fill it with random characters
    char *buffer = (char*) malloc(packet_size);
    memset(buffer, 0, packet_size);
    for (int i = 0; i < payload_size; i++) {
      buffer[sizeof(struct iphdr) + sizeof(struct icmp_header) + i] = rand() % 256;
    }

    // Copy the buffer data into the packet and send it
    memcpy(packet + sizeof(struct iphdr) + sizeof(struct icmp_header), buffer + sizeof(struct iphdr) + sizeof(struct icmp_header), payload_size);
    ...
    // Receive the packet
    if ((rf = recvfrom(sockfd, packet, packet_size, 0, (struct sockaddr*) &servaddr, &ls)) < 0) {
      perror("recvfrom");
      return 1;
    }
