...
int main(int argc, char **argv) {
  ...
  while (sent < 100) {
    ...
    // Fill the packet buffer with random characters
    char *buffer = (char*) malloc(packet_size - sizeof(struct iphdr) - sizeof(struct icmp_header));
    memset(buffer, 0, packet_size - sizeof(struct iphdr) - sizeof(struct icmp_header));
    for (int i = 0; i < payload_size; i++) {
      buffer[i] = rand() % 256;
    }

    // Copy the payload to the packet buffer
    memcpy(packet + sizeof(struct iphdr) + sizeof(struct icmp_header), buffer, payload_size);
