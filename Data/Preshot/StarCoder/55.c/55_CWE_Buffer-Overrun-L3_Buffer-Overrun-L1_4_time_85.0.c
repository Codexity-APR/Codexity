...
int main() {
  ...
  memset(word, 0, sizeof(word)); // Initialize word array with null characters
  char guessed_letters[MAX_WORD_LENGTH + 1];
  memset(guessed_letters, 0, sizeof(guessed_letters)); // Initialize guessed letters array with null characters
  int num_guesses = 0;
  ...
  // Copy the randomly chosen word to the "word" array
