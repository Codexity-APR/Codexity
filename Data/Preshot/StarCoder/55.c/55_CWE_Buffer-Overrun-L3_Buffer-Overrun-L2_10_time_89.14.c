...
int main() {
  ...
  memset(word, '\0', sizeof(word)); // initialize word with null characters
  char guessed_letters[MAX_WORD_LENGTH];
  memset(guessed_letters, '\0', sizeof(guessed_letters)); // initialize guessed_letters with null characters
  int num_guesses = 0;
  ...
  if (word_length > MAX_WORD_LENGTH) {
    printf("Word length exceeds maximum allowed length.");
    return 1;
  }

  // Copy the chosen word to the word array
