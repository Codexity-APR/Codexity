...
#include <string.h> // need this for strncpy
...
void addClient(char *name, int birthday, int landline, int cellphone){
    clientdata *newclient = (clientdata *) malloc(sizeof(clientdata)); //allocate memory for the new client
    if (newclient != NULL) { //check if memory allocation was successful
        strncpy(newclient->name, name, sizeof(newclient->name)); //copy the name into the new client
...
void editClient(int clientnum, char *name, int birthday, int landline, int cellphone){
    ...
    while(current!= NULL){ //while the current client is not NULL
        if(current->clientnum == clientnum){ //if the current client number is equal to the client number to edit
            strncpy(current->name, name, sizeof(current->name)); //copy the name into the current client
