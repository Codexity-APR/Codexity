...
void addClient(char *name, int birthday, int landline, int cellphone){
    clientdata *newclient = NULL; //initialize to NULL
    newclient = (clientdata *) malloc(sizeof(clientdata)); //allocate memory for the new client
    if(newclient != NULL){ //check if allocation was successful
        memcpy(newclient->name, name, strlen(name)); //copy the name into the new client
...
void editClient(int clientnum, char *name, int birthday, int landline, int cellphone){
    ...
    while(current!= NULL){ //while the current client is not NULL
        if(current->clientnum == clientnum){ //if the current client number is equal to the client number to edit
            memcpy(current->name, name, strlen(name)); //copy the name into the current client
