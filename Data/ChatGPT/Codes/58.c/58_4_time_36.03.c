...
static PyObject* read_binary_data(PyObject* self, PyObject* args) {
    ...
    // Create a dictionary to hold the apparms data
    PyObject* apparms_dict = PyDict_New();
    PyDict_SetItemString(apparms_dict, "version", PyLong_FromLong(apparms.version));
    PyDict_SetItemString(apparms_dict, "upgrade", PyLong_FromLong(apparms.upgrade));
